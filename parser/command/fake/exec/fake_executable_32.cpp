...
bool FakeExecutable::RebuildExecDynamic32(LinkMap* executable) {
    if (!executable)
        return false;
    ...
    if (map) {
        ...
        for (int i = 0; i < ehdr->e_phnum; ++i) {
            ...
            lp32::Dynamic *tmp_dynamic = (lp32::Dynamic *)(map->data() + phdr[i].p_offset);
            int numdynamic = phdr[i].p_filesz / sizeof(lp32::Dynamic);
            ...
            idx = 0;
            while (idx < numdynamic) {
                CoreApi::Write(exec_dynamic.Ptr() + idx * sizeof(lp32::Dynamic),
                               (void *)&tmp_dynamic[idx], sizeof(lp32::Dynamic));
                if (tmp_dynamic[idx].type == DT_DEBUG) {
                    lp32::Dynamic dynamic;
                    dynamic.type = DT_DEBUG;
                    dynamic.value = CoreApi::GetDebugPtr();
                    CoreApi::Write(exec_dynamic.Ptr() + idx * sizeof(lp32::Dynamic),
                                   (void *)&dynamic, sizeof(lp32::Dynamic));
                }
                idx++;
            }
        }
        ...
        auto fake_auxv_callback = [&](::Auxv* auxv) -> bool {
            if (auxv->type() == AT_PHDR) {
                auxv->setValue(executable->l_addr() + sizeof(Elf32_Ehdr));

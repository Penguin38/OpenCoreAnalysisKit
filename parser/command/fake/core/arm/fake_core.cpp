...
int FakeCore::execute(const char* output) {
    ...
    uint32_t core_size = RoundUp(note.p_offset + note.p_filesz, align_size) + ehdr.e_shentsize * ehdr.e_shnum;
    ...
    if (stream->Libs().size()) {
        ...
        uint32_t fake_vma = CoreApi::NewLoadBlock(0x0, page_size *
        ...
        uint32_t fake_dynamic = fake_vma + page_size * FAKE_PHDR_PAGES;
        uint32_t fake_link_map = fake_dynamic + page_size * FKAE_DYNAMIC_PAGES;
        uint32_t fake_strtab = fake_link_map + page_size * FAKE_LINK_MAP_PAGES;
...
uint32_t FakeCore::WriteCorePrStatus(std::unique_ptr<MemoryMap>& map, uint32_t off) {
    ...
    while (index < prnum) {
        ...
        memcpy(reinterpret_cast<void *>(map->data() + off + tmp_off), (void *)&prstatus[index], sizeof(Elf32_prstatus));
        tmp_off += sizeof(Elf32_prstatus);
        index++;
    }

...
bool TombstoneParser::parseRegister() {
    ...
    while (fgets(kLine, sizeof(kLine), mFp)) {
        ...
        fgets(kLine, sizeof(kLine), mFp);
        sscanf(kLine, " r8 %x r9 %x r10 %x r11 %x", &regs.regs[8], &regs.regs[9], &regs.regs[10], &regs.regs[11]);

        fgets(kLine, sizeof(kLine), mFp);
        sscanf(kLine, " r12 %x sp %x lr %x pc %x cpsr %x", &regs.regs[12], &regs.sp, &regs.lr, &regs.pc, &regs.cpsr);

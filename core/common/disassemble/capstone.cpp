...
void Disassember::Dump(const char* prefix, uint8_t* begin, uint32_t size, uint64_t address, Option& opt) {
    ...
    if (count > 0) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (arch == CS_ARCH_X86) {
                LOGI("%s" ANSI_COLOR_CYAN "0x%lx: " ANSI_COLOR_YELLOW "%24s" ANSI_COLOR_RESET " | " ANSI_COLOR_GREEN "%s %s\n" ANSI_COLOR_RESET,
                        prefix, insn[i].address, machine_code.c_str(), insn[i].mnemonic, insn[i].op_str);
            } else {
                LOGI("%s" ANSI_COLOR_CYAN "0x%lx: " ANSI_COLOR_YELLOW "%8s" ANSI_COLOR_RESET " | " ANSI_COLOR_GREEN "%s %s\n" ANSI_COLOR_RESET,
                        prefix, insn[i].address, machine_code.c_str(), insn[i].mnemonic, insn[i].op_str);
            }
            num++;
        }
